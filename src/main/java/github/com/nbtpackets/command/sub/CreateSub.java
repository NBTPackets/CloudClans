...
import github.com.nbtpackets.menu.impl.ConfirmCreate;
...
public class CreateSub implements SubCommand {
    ...
    private void validateClanName(Player player, String clanName) {
        int min = CloudClans.getInstance().getConfig().getInt("settings.min-name", 3);
        int max = CloudClans.getInstance().getConfig().getInt("settings.max-name", 8);
