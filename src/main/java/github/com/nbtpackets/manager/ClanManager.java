package github.com.nbtpackets.manager;
...
public class ClanManager implements ClanProvider {
    ...
    public void load() {
        db.loadAll().thenAccept(loaded -> {
            ...
            playerToClan.clear();
            GlowManager.clear();
            ClanLevelService levelService = CloudClans.getInstance().getLevelService();
    ...
    public void startInviteCleanupTask() {
        Task.timer(20L * 60L, 20L * 60L, () -> {
            int timeout = CloudClans.getInstance().getConfig().getInt("settings.invite-timeout", 60);
            long now = System.currentTimeMillis();
    ...
    public void addExp(UUID playerUuid, long amount) {
        ...
        getPlayerClan(playerUuid).ifPresent(clan -> {
            ...
            int  newLevel = CloudClans.getInstance().getLevelService().calculateLevel(newExp);
            ...
            if (newLevel > oldLevel) {
                ...
                CloudClans.getInstance().getLevelService().handleLevelUp(updated, oldLevel);
            }
        });
    }
